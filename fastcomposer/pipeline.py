...
class StableDiffusionFastCompposerPipeline(StableDiffusionPipeline):
    ...
    def _encode_augmented_prompt(
        ...
        for image in reference_images:
            image_tensor = torch.from_numpy(np.array(image.convert("RGB"))).permute(
                2, 0, 1
            )
        ...
        object_pixel_values = (
            torch.stack(object_pixel_values, dim=0)
    ...
    def __call__(
        ...
        (null_prompt_embeds, text_prompt_embeds, augmented_prompt_embeds) = (
